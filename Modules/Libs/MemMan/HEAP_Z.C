...
{
	system_heap.start = (heap_mcb *)ucHeap;
	system_heap.hsize = sizeof( ucHeap );
...
//	if( ( xptr->ts.type == MARK_FREE )&&( xptr != heap->start ) )  	// Баг найденный Сергеем Борщем
	...
	if( ( xptr->ts.type == MARK_FREE )&&( xptr == ( tptr + sizeof(heap_mcb) + tptr->ts.size ) ) )
...
//  if( ( xptr->ts.type == MARK_FREE )&&( tptr != heap->start ) ) 	// Аналогичный баг
    // Если предыдущий MCB свободен и примыкает к текущему
  	if( ( xptr->ts.type == MARK_FREE )&&( tptr == ( xptr + sizeof(heap_mcb) + xptr->ts.size ) ) )
